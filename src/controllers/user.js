...
const signupSchema = Joi.object({
    ...
    email: Joi.string().regex(/^\S+@\S+\.\S+$/).required().messages({
        'string.pattern.base': 'Invalid email format? required: johndoe@example.com'
    }),
    ...
    try {
        ...
        db.query('SELECT * FROM users WHERE email = ?', [email], async (err, result) => {
            if (err) {
                ...
                return res.status(400).json({ message: "Error occurred during signup" });
            }
            ...
            if (result.length > 0) {
                ...
                db.query('INSERT INTO users (firstName, lastName, email, password, saltKey, userType, createdAtIP, updatedAtIP) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', 
...
const login = async (req, res) => {
    ...
    try{
        ...
        db.query('SELECT * FROM users WHERE email = ? AND deletedAt IS NULL', [email], async (err, results) => { 
            if (err) {
                ...
                if (!passwordMatch) {
                    return res.status(401).json({ error: "Incorrect Password" });
                        ...
                        const authToken = generateToken({ id: user.Id, userId: user.id, userType: user.userType }, '1h');

                        return res.status(200).json({ message: "Login successful", Email: user.email, authToken });
                  }
...
const getAllUser = (req, res) => {
    try{
        ...
        db.query('SELECT id, firstname, lastname, email FROM users', (err, results) => {
            if (err) {
                console.log("Server error about query", err);
                return res.json({ error: err });
              } else {
...
const updateUserDetail = async (req, res) =>{
    ...
    try {
        db.query('SELECT * FROM users WHERE id = ?', [userId], async (err, results) => {
            ...
            db.query('UPDATE users SET firstName = ?, lastName = ?, password = ?, updatedAtIP = ? WHERE id = ?',
...
const deleteUser = async (req, res) =>{
    ...
    try {
        db.query('UPDATE users SET deletedAt = CURRENT_TIMESTAMP() WHERE id = ?', [userId], async (err, results) => {
            if (err) {
                console.error("Server error about query", err);
                return res.status(400).json({ error: "Server Error" });
            }
